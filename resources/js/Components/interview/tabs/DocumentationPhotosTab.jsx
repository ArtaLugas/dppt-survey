    ...
    const groupedData = useMemo(
        () =>
            photoTypes.map((pt) => {
                ...
                const typePhotos = safePhotos.filter(
        ...
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-6">
                <div className="space-y-1">
                    <h3 className="text-2xl font-black tracking-tight flex items-center gap-3 text-foreground">
                        <Camera className="text-primary h-7 w-7" /> Manajemen
                        Visual
                    </h3>
                    <p className="text-muted-foreground text-sm max-w-xl">
                        Pusat kendali bukti lapangan. Sistem otomatis
                        mengekstrak titik koordinat (Geotagging) dari metadata
                        foto untuk validasi lokasi.
                    </p>
                </div>
                <Button
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {groupedData.map((group) => {
                    ...
                    return (
                        <Card
                            key={`widget-${group.type.id}`}
                            className="border-none bg-muted/30 rounded-[2rem] overflow-hidden shadow-sm hover:shadow-md transition-all group"
                        >
                        ...
                        >
                            ...
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-l-4 border-primary pl-6 py-2 bg-gradient-to-r from-muted/30 to-transparent rounded-r-2xl">
                                <div className="flex items-center gap-4">
                                    <div
                                        className={cn(
                                            "p-3 rounded-xl shadow-sm bg-background border",
                                            meta.color,
                                            meta.border,
                                        )}
                                    >
                                ...
                                <Badge
                                    variant={group.met ? "default" : "outline"}
                                    className={cn(
                                        "px-5 py-2 font-bold tracking-wide rounded-full shadow-sm",
                                        group.met
                            ...
                            {group.photos.length > 0 ? (
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                                    {group.photos.map((photo) => (
                                        <Card
                                            key={photo.id}
                                            className="group relative flex flex-col overflow-hidden rounded-[1.5rem] border border-border/60 bg-card shadow-sm transition-all duration-500 hover:shadow-2xl hover:border-primary/30 hover:-translate-y-1"
                                        >
                                            {/* Area Gambar */}
                                            <div className="relative aspect-[4/3] w-full overflow-hidden bg-muted/50">
                                                    ...
                                                    <img
                                                        ...
                                                        onError={(e) => {
                                                            e.target.onerror =
                                                                null;
                                                            e.target.src =
                                                                "https://placehold.co/600x400/f3f4f6/9ca3af?text=File+Corrupt";
                                                        }}
                                                    />
                                                ) : (
                                                ...
                                                photo.longitude ? (
                                                    <div className="absolute bottom-3 left-3 right-3 flex justify-center">
                                                        <div className="flex max-w-full items-center gap-2 rounded-full border border-white/30 bg-black/70 px-3 py-1.5 shadow-xl backdrop-blur-md transition-all group-hover:bg-black/90">
                                                            <MapPin className="h-3.5 w-3.5 shrink-0 text-white" />
                                                            <span className="truncate font-mono text-[10px] font-bold text-white drop-shadow-md">
                                                                {Number(
                                                                    photo.latitude,
                                                                ).toFixed(6)}
                                                                ,{" "}
                                                    ...
                                                    <div className="absolute bottom-3 left-3">
                                                        <Badge
                                                            variant="destructive"
                                                            className="border-none bg-red-600 text-white text-[10px] shadow-lg backdrop-blur-md pointer-events-none"
                                                        >
                                                            No GPS Data
                                                        </Badge>
                                                    </div>
                                            ...
                                            <div className="flex flex-1 flex-col justify-between p-5">
                                                <div className="space-y-2">
                                                    <div className="flex items-start justify-between gap-2">
                                                        <h5
                                                            className="line-clamp-1 text-sm font-bold text-foreground"
                                                            title={
                                                                photo.file_name
                                                            }
                                                        >
                                                        ...
                                                        <Badge
                                                            variant="secondary"
                                                            className="shrink-0 text-[9px] uppercase font-bold tracking-wider text-muted-foreground bg-muted"
                                                        >
                                                            {photo.mime_type
                                                                ? photo.mime_type.split(
                                                                      "/",
                                                                  )[1]
                                                                : "IMG"}
                                                        </Badge>
                                    ...
                                    {canModify && (
                                        ...
                                        >
                                            ...
                                            <p className="mt-4 text-sm font-bold text-muted-foreground group-hover:text-primary">
                                                Tambah {group.type.label}
                                            </p>
                                        </button>
                                    )}
                                </div>
                                ...
                                <div className="relative flex w-full flex-col items-center justify-center overflow-hidden rounded-[2.5rem] border border-dashed border-border bg-muted/5 py-20 text-center transition-all hover:bg-muted/10">
                                    <div
                                        className={cn(
                                            "mb-6 flex h-24 w-24 items-center justify-center rounded-[2rem] bg-gradient-to-br shadow-inner",
                                            meta.bg,
                                        )}
                                    >
                                        {meta.icon}
                                    ...
                                    <p className="mt-2 max-w-sm text-sm font-medium text-muted-foreground">
                                        ...
                                        mewajibkan minimal{" "}
                                        {getMinQty(group.type)} foto untuk
                                        verifikasi validitas bidang.
                                    </p>
                                    {canModify && (
                                        <Button
                                            onClick={() => setDialogOpen(true)}
                                            variant="outline"
                                            className="mt-8 rounded-full border-primary/20 px-8 font-bold text-primary hover:bg-primary hover:text-white"
                                        >
