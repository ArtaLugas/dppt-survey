...
import {
  ArrowLeft, Lock, AlertCircle, Users, MapPin,
  Package, Camera, FileText, CheckCircle2, ShieldCheck, XCircle, AlertTriangle
} from 'lucide-react';
...
import { useInterviewAccess, getStatusLabel, getStatusColor } from '@/hooks/useInterviewAccess';
...
const TABS = [
  { id: 'info',        label: 'Bidang',       icon: <FileText className="h-4 w-4" />, requiresSaved: false },
  { id: 'respondents', label: 'Responden',    icon: <Users    className="h-4 w-4" />, requiresSaved: true  },
  { id: 'land-detail', label: 'Detail Tanah', icon: <MapPin   className="h-4 w-4" />, requiresSaved: true  },
  { id: 'inventory',   label: 'Inventaris',   icon: <Package  className="h-4 w-4" />, requiresSaved: true  },
  { id: 'photos',      label: 'Foto',         icon: <Camera   className="h-4 w-4" />, requiresSaved: true  },
];
...
export default function InterviewEditor({
    ...
    const userRoleString =
        ...
        (currentUser?.role_id === 3 ? 'surveyor'    :
        (currentUser?.role_id === 2 ? 'koordinator' :
        (currentUser?.role_id === 1 ? 'admin'       : 'unknown')));

    const accessState = useInterviewAccess({
        interview: parcel,
        userRole:  userRoleString,
        isOwner:   Number(parcel?.surveyor_id) === Number(currentUser.id) || isCreateMode,
    });
    ...
    const photoValidation = useMemo(() => {
        ...
        const isAllValid = photoTypes.every(pt => {
            const count       = photos.filter(p => p.photo_type_id === pt.id || p.photoTypeId === pt.id).length;
    ...
    const readinessChecklist = useMemo(() => [
        ...
        { id: 'respondents', label: 'Data Responden',  met: respondents.length > 0  },
        { id: 'landDetail',  label: 'Detail Tanah',    met: !!landDetail             },
        { id: 'inventory',   label: 'Inventaris',      met: inventoryItems.length > 0 },
    ...
    const handleSaveParcelInfo = (data, callbacks = {}) => {
        ...
        router[method](url, data, {
            ...
            onSuccess: (page)   => {
                toast({ title: 'Tersimpan', description: 'Data informasi bidang berhasil disimpan.' });
    ...
    const handleSubmit = () => {
        if (!canSubmit) return;
        router.post(route('surveyor.interviews.submit', parcel.id), {}, {
            preserveState: true,
            onStart:   ()       => setIsSaving(true),
            onSuccess: ()       => {
                toast({ title: 'Verifikasi Terkirim', description: 'Data berhasil disubmit dan dikunci untuk proses review.' });
                setShowSubmitDialog(false);
                handleBack();
            },
            onError:   (errors) => toast({
                title: 'Gagal Submit', description: Object.values(errors)[0] || 'Terjadi kesalahan.', variant: 'destructive',
            }),
            onFinish:  ()       => setIsSaving(false),
        });
    };
    ...
    const handleAddRespondent = (data, callbacks = {}) => {
        router.post(route('surveyor.interviews.respondents.store', parcel.id), data, {
            preserveScroll: true,
            onSuccess: (page)   => {
                toast({ title: 'Berhasil', description: 'Responden ditambahkan.' });
                if (callbacks.onSuccess) callbacks.onSuccess(page);
            },
            onError:   (errors) => {
                toast({ title: 'Gagal', description: 'Cek input Anda.', variant: 'destructive' });
                if (callbacks.onError) callbacks.onError(errors);
            },
        });
    };
    ...
    const handleEditRespondent = (id, data, callbacks = {}) => {
        router.put(route('surveyor.interviews.respondents.update', id), data, {
            preserveScroll: true,
            onSuccess: (page)   => {
                toast({ title: 'Berhasil', description: 'Data responden diperbarui.' });
                if (callbacks.onSuccess) callbacks.onSuccess(page);
            },
            onError:   (errors) => {
                ...
                toast({
                    ...
                    description: Object.values(errors).join(' | ') || 'Gagal menyimpan ke database.',
                    variant:     'destructive',
                });
                if (callbacks.onError) callbacks.onError(errors);
            },
    ...
    const handleDeleteRespondent = (id) => {
        router.delete(route('surveyor.interviews.respondents.destroy', id), {
            preserveScroll: true,
            onSuccess: () => toast({ title: 'Berhasil', description: 'Responden dihapus.' }),
        });
    };
    ...
    const handleSaveLandDetail = (data, callbacks = {}) => {
        router.post(route('surveyor.interviews.land-details.store', parcel.id), data, {
            preserveScroll: true,
            onStart: () => {
                if (callbacks.onStart) callbacks.onStart();
            },
            onSuccess: (page) => {
                toast({ title: 'Berhasil', description: 'Detail bidang tanah berhasil disimpan.' });
                if (callbacks.onSuccess) callbacks.onSuccess(page);
            },
            onError: (err) => {
    ...
    const handleDeleteLandDetail = (callbacks = {}) => {
        router.delete(route('surveyor.interviews.land-details.destroy', parcel.id), {
            preserveScroll: true,
            onSuccess: () => {
                toast({ title: 'Berhasil', description: 'Detail tanah telah dikosongkan.' });
    ...
    const handleAddInventory = (data, callbacks = {}) => {
        router.post(route('surveyor.interviews.inventory-items.store', parcel.id), data, {
            preserveScroll: true,
            onSuccess: (page)   => {
                toast({ title: 'Berhasil', description: 'Aset baru berhasil ditambahkan.' });
                if (callbacks.onSuccess) callbacks.onSuccess(page);
            },
            onError:   (errors) => {
                toast({ title: 'Gagal', description: 'Periksa kembali isian form anda.', variant: 'destructive' });
                if (callbacks.onError) callbacks.onError(errors);
            },
        });
    };
    ...
    const handleEditInventory = (itemId, data, callbacks = {}) => {
        router.put(route('surveyor.interviews.inventory-items.update', itemId), data, {
            preserveScroll: true,
            onSuccess: (page)   => {
                toast({ title: 'Berhasil', description: 'Data aset diperbarui.' });
                if (callbacks.onSuccess) callbacks.onSuccess(page);
            },
            onError:   (errors) => {
    ...
    const handleDeleteInventory = (itemId) => {
        router.delete(route('surveyor.interviews.inventory-items.destroy', itemId), {
            preserveScroll: true,
            onSuccess: () => toast({ title: 'Terhapus', description: 'Data aset telah dihapus.' }),
        });
    };
    ...
    const handleAddPhoto = (data, callbacks = {}) => {
        router.post(route('surveyor.interviews.documentation-photos.store', parcel.id), data, {
            forceFormData:  true,
            preserveScroll: true,
            onSuccess: (page)   => {
            ...
            onError:   (errors) => {
                toast({
                    ...
                    description: Object.values(errors)[0] || 'Periksa koneksi.',
                    variant:     'destructive',
                });
                if (callbacks.onError) callbacks.onError(errors);
            },
    ...
    const handleDeletePhoto = (photoId, callbacks = {}) => {
        router.delete(route('surveyor.interviews.documentation-photos.destroy', photoId), {
            preserveScroll: true,
            onSuccess: () => {
                toast({ title: 'Terhapus', description: 'Foto dokumentasi telah dihapus.' });
                if (callbacks.onSuccess) callbacks.onSuccess();
            },
            onFinish: () => {
                if (callbacks.onFinish) callbacks.onFinish();
    ...
    // ── RENDER ────────────────────────────────────────────────────────────────
    return (
        <DashboardLayout userName={currentUser.name} userRole={currentUser.role}>
            <Head>
                <title>
                    {isCreateMode
            ...
            <div className="w-full space-y-8 animate-in fade-in duration-500 pb-10">
                ...
                <div className="sticky top-[-30px] z-40 -mx-6 px-6 py-4 mb-6 bg-background/95 backdrop-blur-md border-b border-border/60 transition-all duration-200">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                            <Button variant="ghost" size="icon" onClick={handleBack} className="rounded-full hover:bg-muted/80">
                                <ArrowLeft className="h-5 w-5" />
                            </Button>
                            <div>
                                <h1 className="text-xl md:text-2xl font-black tracking-tight text-foreground">
                                    {isCreateMode ? 'Registrasi Bidang Baru' : 'Berkas Bidang'}
                                </h1>
                                {parcel && (
                                    <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                                        ...
                                        <Badge className={cn('px-2 py-0.5 text-[10px] font-bold shadow-sm', getStatusColor(parcel.status_id))}>
                                            {getStatusLabel(parcel.status_id)}
                                        </Badge>
                                        <span className="hidden sm:inline">•</span>
                                        {parcel.updated_at && (
                                            <span className="hidden sm:inline">
                                                Update: {format(new Date(parcel.updated_at), 'dd MMM yyyy • HH:mm', { locale: idLocale })}
                                            </span>
                                        )}
                                    </div>
                                )}
                            ...
                            {parcel && accessState.canSubmit && (
                                <Button
                                    ...
                                    className={cn(
                                        ...
                                        canSubmit ? "bg-primary hover:bg-primary/90" : "bg-muted text-muted-foreground shadow-none"
                                    )}
                                >
                                    {canSubmit
                ...
                {accessState.isFinal && (
                    <Alert variant="destructive" className="bg-red-50 border-red-200 text-red-800 rounded-2xl">
                        ...
                        <div className="ml-2">
                            <AlertTitle className="font-bold text-red-900">Berkas Terkunci (Read-Only)</AlertTitle>
                            <AlertDescription className="text-red-700 mt-1">
                                Data ini sudah disubmit atau diverifikasi. Anda tidak dapat lagi melakukan perubahan.
                            </AlertDescription>
                        </div>
                    </Alert>
                )}
                            ...
                            <div className="flex flex-col xl:flex-row gap-6 items-start xl:items-center justify-between">
                                <div className="space-y-1 max-w-lg">
                                    <h3 className="text-base font-black flex items-center gap-2">
                                        ...
                                        {canSubmit ? 'Data Siap Dikirim' : 'Status Kelengkapan Berkas'}
                                    </h3>
                                ...
                                <div className="flex flex-wrap gap-3 w-full xl:w-auto xl:justify-end">
                                    {readinessChecklist.map((item) => (
                                        <div key={item.id} className={cn(
                                            ...
                                            item.met ? "border-emerald-200" : "border-red-200 animate-pulse"
                                        )}>
                                            {item.met
                                            ...
                                            <span className={cn(
                                                ...
                                                item.met ? "text-foreground" : "text-red-700"
                                            )}>
                                                {item.label}
                                            </span>
                                        </div>
                ...
                <div className="w-full">
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full space-y-8">
                        ...
                        <div className="w-full">
                            <TabsContent value="info" className="focus-visible:outline-none mt-0">
                                <ParcelInfoTab parcel={parcel} accessState={accessState} onSave={handleSaveParcelInfo} isLoading={isSaving} />
                            </TabsContent>

                            <TabsContent value="respondents" className="focus-visible:outline-none mt-0">
                                {parcel
                                    ? <RespondentsTab parcelId={parcel.id} respondents={respondents} respondentRoles={respondentRoles} accessState={accessState} onAdd={handleAddRespondent} onEdit={handleEditRespondent} onDelete={handleDeleteRespondent} />
                                    : <EmptyMsg />}
                            </TabsContent>

                            <TabsContent value="land-detail" className="focus-visible:outline-none mt-0">
                                {parcel
                                    ? <LandDetailTab parcelId={parcel.id} landDetail={landDetail} accessState={accessState} onSave={handleSaveLandDetail} onDelete={handleDeleteLandDetail} />
                                    : <EmptyMsg />}
                            </TabsContent>

                            <TabsContent value="inventory" className="focus-visible:outline-none mt-0">
                                {parcel
                                    ? <InventoryTab parcelId={parcel.id} items={inventoryItems} accessState={accessState} onAdd={handleAddInventory} onEdit={handleEditInventory} onDelete={handleDeleteInventory} />
                                    : <EmptyMsg />}
                            </TabsContent>

                            <TabsContent value="photos" className="focus-visible:outline-none mt-0">
                                {parcel
                                    ? <DocumentationPhotosTab parcelId={parcel.id} photos={photos} photoTypes={photoTypes} accessState={accessState} onAdd={handleAddPhoto} onDelete={handleDeletePhoto} />
                                    : <EmptyMsg />}
                            </TabsContent>
                        </div>
                    </Tabs>
                ...
                <Dialog open={showSubmitDialog} onOpenChange={setShowSubmitDialog}>
                    <DialogContent className="max-w-md p-0 overflow-hidden bg-background sm:rounded-[2rem] border-none shadow-2xl">
                        <div className="bg-primary/10 p-8 flex flex-col items-center justify-center text-center border-b border-primary/20">
                            ...
                            <DialogTitle className="text-2xl font-black text-foreground">Deklarasi Integritas Data</DialogTitle>
                        </div>

                        <div className="p-8 space-y-6">
                            <DialogDescription className="text-base text-foreground font-medium leading-relaxed">
                                Dengan menekan tombol submit, Anda menyatakan bahwa:
                                <ul className="list-disc pl-5 space-y-2 mt-4 text-muted-foreground text-sm">
                                    <li>Data identitas, ukuran, dan aset diisi sesuai fakta di lapangan.</li>
                                    <li>Foto dokumentasi bersifat asli tanpa manipulasi (GPS).</li>
                                    <li>Data ini akan dikunci untuk verifikasi lanjutan.</li>
                                </ul>
                            </DialogDescription>

                            <div className="flex gap-3 pt-4 border-t border-border">
                                <Button type="button" variant="ghost" className="w-full font-bold h-12 rounded-xl" onClick={() => setShowSubmitDialog(false)}>
                                    Batal
                                </Button>
                                <Button onClick={handleSubmit} disabled={isSaving} className="w-full font-bold h-12 rounded-xl bg-primary hover:bg-primary/90 shadow-lg shadow-primary/20">
                                    {isSaving ? 'Memproses...' : 'Ya, Submit Berkas'}
                                </Button>
                            </div>
                        </div>
                    </DialogContent>
...
function EmptyMsg() {
    return (
        <div className="flex flex-col items-center justify-center py-24 text-center border-2 border-dashed border-muted rounded-[2rem] bg-muted/5 w-full">
            ...
            <h3 className="text-lg font-bold text-foreground">Akses Terkunci</h3>
            <p className="text-sm text-muted-foreground max-w-xs mt-2">
                Anda harus mengisi dan menyimpan Informasi Dasar Bidang terlebih dahulu pada Tab Bidang.
            </p>
        </div>
    );
}
